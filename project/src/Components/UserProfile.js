...
const UserProfile = () => {
  useEffect(()=>{
    document.title=`Bistro Bliss UserProfile` 
   })
...
{DataUsers && (
            <div className="card-profile">
              <h1
                className="usernamexx"
                style={{ backgroundColor: `hsl(${random}, 70%, 54%)` }}
              >
                {DataUsers?.username.split("").splice(0, 1).join(" ")}
              </h1>
              <h3 className="banogoo">{DataUsers?.username}</h3>
              <h3 className="banogoo">{DataUsers?.email}</h3>
              ...
              <Link to="/" className="Vostaros">
          <button className="buttonzx" onClick={() => window.scrollTo(0, 0)}>Go Back</button>
              </Link>
            </div>
          )}
  </div>
